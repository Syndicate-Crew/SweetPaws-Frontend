<template>
	<b-container class="tab-view-container w-100">
		<b-form @submit.prevent="" class="w-100">
			<b-col class="form-group-column">
				<b-form-group class="form-group image-from-group">
					<h4 class="change-image-h2">Change the Profile image</h4>
		...
		<b-form @submit.prevent="handleSubmitInfo" class="w-100">
			<h4 class="change-password-h2" style="margin-top: 30px; margin-bottom: 10px">
				Change Info
			</h4>
			<b-col class="form-group-column">
		...
		<b-form @submit.prevent="" class="w-100">
			<b-col class="form-group-column">
				<h4 class="change-password-h2">Change Password</h4>
				<b-form-group class="form-group">
					<b-row class="w-100 form-group-lable">
					...
					<b-row class="form-actions w-100">
						<button class="form-btn" @click.prevent="resetPassword">
							Reset
						</button>
						<button
							class="form-btn"
	...
	mounted() {
		if (localStorage.getItem("sweet-token-admin") == null)	{
			...
			.then((result) => {
				...
				this.user.name = result.data.result.name;
				this.user.phone = result.data.result.phone;

				this.defaults.email = result.data.result.email;
				this.defaults.name = result.data.result.name;
				this.defaults.phone = result.data.result.phone;

				this.imageUpload.url = `http://localhost:5000/api/public/profile_pictures/${result.data.result.image}`;
			})
			.catch((err) => {
				console.log(err);
			});
