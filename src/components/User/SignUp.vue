	...
	<b-container fluid class="w-75 form-wrapper">
				...
				<b-row class="form-col-row" cols-sm="1" cols="2" cols-md="2">
					<b-col>
						...
						<div style="font-size: 13px;width: 300px">
							Hover and click the upload button to upload the profile
							picture(.png and .jpg only)
						</div>
						<div
							v-if="image == '../../assets/person.png'"
							style="font-size: 13px;color: red;width: 300px"
							:class="{ 'is-invalid': image == '../../assets/person.png' }"
						>
							Upload an Image
						</div>
					</b-col>
		...
		<div class="form-hint">
			<center>
				...
				<span class="form-hint-link" @click="window.location = ''"
					>Sign in</span
				>
			</center>
		</div>
		...
		handleSubmit() {
			...
			if (this.$v.$invalid || this.image == "../../assets/person.png") {
				...
				const form = new FormData();
					...
					this.axios
						.post("http://localhost:5000/api/user/SignUp", form)
						.then((result) => {
